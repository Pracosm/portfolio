...
4. **Voiceover**: `vo_lines.json` holds the script with a start time per line. `vo_build.py` voices each line with edge-tts (`en-US-AndrewMultilingualNeural`, +12% rate; pass another voice name as the first argument) through `tts.py`, trims the silence, and places each line at its cut without overlaps.
