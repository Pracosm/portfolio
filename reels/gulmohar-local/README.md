...
4. **Voiceover**: `vo_lines.json` holds the script with a start time per line. `vo_build.py` voices each line with edge-tts (`en-IN-PrabhatNeural`, +20% rate) through `tts.py`, trims the silence, and places each line at its cut without overlaps.
   Mix: the music is ducked under the voice with `sidechaincompress`, then loudness is normalised to -14 LUFS.
